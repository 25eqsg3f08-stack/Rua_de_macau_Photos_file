// 配置项
const CONFIG = {
    // GitHub 仓库 tree 页面（用于解析图片列表）
    REPO_TREE_URL: "https://github.com/25eqsg3f08-stack/Rua_de_macau_Photos/tree/main",
    // 原始图片地址前缀
...
// 从 GitHub tree 页面解析图片文件名（核心替代 API 的方法）
async function fetchPhotoList() {
    try {
        el.loading.textContent = "正在获取仓库图片列表...";
        // 1. 请求 tree 页面 HTML（需借助 CORS 代理，否则浏览器会拦截跨域请求）
        const corsProxy = "https://api.allorigins.win/get?url=";
        const targetUrl = corsProxy + encodeURIComponent(CONFIG.REPO_TREE_URL);
        const res = await fetch(targetUrl);
        if (!res.ok) throw new Error("获取仓库页面失败");
        const data = await res.json();
        const html = data.contents;

        // 2. 解析 HTML 提取图片文件名（匹配 jpg/png/jpeg/webp 后缀）
        const imgRegex = /href="\/25eqsg3f08-stack\/Rua_de_macau_Photos\/blob\/main\/([^"]+\.(jpg|png|jpeg|webp))"/gi;
        const fileNames = [];
        let match;
        ...
        // 3. 去重并生成完整图片 URL
        photoList = [...new Set(fileNames)].map(name => CONFIG.RAW_BASE_URL + name);
        ...
        el.loading.textContent = "无法获取仓库图片，请检查网络或稍后重试";
        // 本地调试备用：取消注释手动添加图片
        // photoList = ["https://picsum.photos/800/600?random=1"];
    }
}
...
function bindEvents() {
    ...
    // 打印按钮跳转到 GitHub Pages 地址，传递当前图片URL
